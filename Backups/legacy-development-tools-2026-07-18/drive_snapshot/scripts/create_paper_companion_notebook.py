...
- **Imaging study:** 19 GCaMP6s mice, 89 physical recordings and 20,547–89,577
  detected neurons per recording. Rewarded task mice were compared with mice
  given unrewarded exposure, naive controls and grating-exposure controls.
...
belong to more than one analysis label. Therefore, the 142 index memberships
are **not** 142 independent recordings and figure sample sizes are not additive.
"""
        ),
        py(
            """
...
print(
    ...
    f"{index_summary['associations']} label memberships"
)
...
"""
        ...
        py(
            """
# A schematic, not measured data.
fig, ax = plt.subplots(figsize=(11, 2.5), constrained_layout=True)
ax.barh(0, 4, left=0, height=0.55, color="#59a14f", label="textured corridor")
ax.barh(0, 2, left=4, height=0.55, color="#b8b8b8", label="grey corridor")
ax.axvspan(0.5, 3.5, ymin=0.18, ymax=0.82, color="#f28e2b", alpha=0.18)
ax.text(2, 0.02, "visual texture", ha="center", va="center", weight="bold")
ax.text(5, 0.02, "grey", ha="center", va="center", weight="bold")
ax.text(2, 0.48, "cue could occur here", ha="center", color="#a14e00")
ax.annotate(
    "water only in the rewarded corridor",
    xy=(3.45, -0.05), xytext=(4.1, -0.55),
    arrowprops={"arrowstyle": "->", "color": "#4c78a8"},
    color="#4c78a8",
)
ax.set(xlim=(0, 6), ylim=(-0.75, 0.7), xlabel="corridor position (m)")
ax.set_yticks([])
...
|---|---|---|---|
| **Fig. 1 — Train 1** | About two weeks of rewarded training or matched unrewarded exposure to familiar textures | Per-neuron corridor selectivity, $d'$, mapped across visual areas | Medial-HVA selectivity increased in both task and unrewarded mice; the strongest reward-specific difference was anterior |
| **Fig. 2 — Test 1** | New crops from the same categories (`leaf2`, `circle2`) | Position sequences, held-out coding direction and similarity | Generalization followed visual features rather than absolute corridor position |
| **Fig. 3 — Train 2** | `leaf2` changed from novel to familiar | Novelty responses, `leaf1`–`leaf2` selectivity and orthogonalization | Novelty responses declined in V1/lateral areas; fine discrimination emerged strongly in medial areas with or without reward |
...
| **Fig. 4 — Reward signal** | Early versus late cues, licked versus unlicked trials, and later tests | Rastermap discovery followed by a late-versus-early-cue $d'$ | A ramping reward-prediction population appeared mainly in anterior HVAs of task mice |
| **Fig. 5 — Behavioural validation** | 10 days of natural-texture, grating or no pretraining before a five-day task | Lick discrimination, first licks and trials per day | Relevant natural-texture pretraining accelerated early rewarded learning |
...
PAPER_STEPS = {
    "fig1": {
        ...
        "changed": "About two weeks of rewarded training or unrewarded exposure.",
...
**Primary sources:** [Nature paper](https://doi.org/10.1038/s41586-025-09180-y)
