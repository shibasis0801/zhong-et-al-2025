#!/usr/bin/env python3
"""Generate the team's reward-versus-exposure d-prime dynamics notebook."""
...
def build_notebook():
    ...
    cells.extend(
        [
            md(
                r"""
# Reward and the within-session dynamics of visual discriminability

This is the analysis notebook for our team question:

> **Does reward make neural discriminability ($d'$) improve faster within one
> training session than matched unrewarded exposure?**

It is written for the whole team. Every major analysis is a **Reaktor-style
executable graph**: labelled functions are nodes, named values are ports, blue
wires show the data flow, and the hollow ports are controls you can change.
Press **Run flow** and read the plots; open the folded code only when you want
the exact calculation.

The notebook keeps **Plan A** (change in $d'$ over trials) and **Plan B**
(activity over corridor position) together because position, speed, cue, lick,
and reward timing are not side issues - they are the main alternative
explanations for a changing neural signal.
"""
            ),
            md(
                r"""
## First: tighten the scientific claim

The release supports a strong and useful comparison, but its name matters:

| Proposed wording | What Train 1 before-learning actually identifies |
|---|---|
| “behaviour-contingent reward” | **Not identified**: all 4 task mice received passive reward |
| “reward present vs absent” | **Supported comparison**: 4 passive-reward mice vs 9 no-reward mice |
| “V1 plasticity” | A valid primary target if pre-registered; mHV/lHV/aHV must be shown separately |
| “caused by reward” | A cohort contrast, not automatic causality; acquisition and behaviour can differ |

The primary notebook label is therefore **Rewarded (passive) vs Unrewarded**.
The after-learning task set mixes passive and active-after-cue reward modes, so
that stage is a sensitivity analysis, not a clean active-reward test.
...
The one-sided prediction is $\Delta_{rate}>0$. Mice - not trials, bins,
components, or neurons - are the independent units in the group contrast.
"""
            ),
            md(
                """
...
| **3 · Analysis sandbox** | 13 nodes · 20 controls | What would faster rate, earlier plateau and uncertainty look like? | No - clearly simulated |
| **4 · Real Plan A** | 25 nodes · 29 controls | Do mouse-level early $d'$ slopes differ between rewarded and unrewarded cohorts? | Yes, only for the locked complete cohort |
| **5 · Plan B corridor** | 14 nodes · 24 controls | Where along the corridor do activity and $d'$ change, and do speed/support explain it? | Formal whole-curve band after a real two-group run |
...
There are deliberately many editable ports. In Graphs 1-3 and 5 they are a
learning laboratory. In Graph 4, every departure from the locked specification
is recorded and the result is labelled preview/sensitivity; controls are not a
license to select the most attractive result.
"""
            ),
            md(
                """
...
The setup works locally in metadata-only mode, so Graphs 1-3 and the bundled
...
"""
            ...
            md(
                """
## What the new neural-analysis review changes here

Stringer and Pachitariu's *Analysis methods for large-scale neuronal
recordings* is in the shared workspace. Four recommendations directly shape
this notebook:

- ask a concrete scientific question before choosing a complex model;
- score encoding/decoding models only on held-out observations;
- use temporally extended splits because slow drift can survive random splits;
- model movement and correlated behaviour before calling a signal sensory,
  cognitive, or reward-related.

Accordingly, the confirmatory trajectory uses **trial-level responses**,
**fixed-width blocks**, **contiguous-fold cross-fitting**, and **mouse-level
inference**. Cumulative-prefix curves remain intuitive pictures only; they are
not used as independent rate observations.

Primary sources: [analysis review](https://doi.org/10.1126/science.adp7429),
[Zhong et al. paper](https://doi.org/10.1038/s41586-025-09180-y), and
[Figshare v2 data](https://doi.org/10.25378/janelia.28811129.v2).
"""
            ),
            md(
                """
...
- A **hollow port** is a setting you can change.
...
fetch_panel = fetch_graph.widget(
    controls={
        "training_set": widgets.Dropdown(
            ...
            options=[("Train 1 (primary)", "train1"), ("Train 2 (generalization)", "train2")],
            value="train1",
        ),
        "stage": widgets.Dropdown(
            description="Training stage",
            options=[("Before learning (primary)", "before_learning"), ("After learning (sensitivity)", "after_learning")],
...
The graph above resolves file names without downloading. When you are ready,
these are the only calls the real analysis uses:
"""
            ),
            py(
                """
...
recording_panel = recording_graph.widget(
    controls={
        ...
        "cortical_area": widgets.Dropdown(
            ...
            options=[("V1 (proposal primary)", "V1"), ("Medial HVA", "mHV"), ("Lateral HVA", "lHV"), ("Anterior HVA", "aHV")],
            value="V1",
        ),
        ...
        "coverage_rule": widgets.Dropdown(
            ...
            options=[("Complete window (primary)", "complete"), ("Any available bin (sensitivity)", "available")],
            value="complete",
        ),
...
"""
    ...
    cells.extend(
        [
            md(
                r"""
## Graph 3 - Plan A analysis sandbox (simulation, never evidence)

This is the team's **what-if laboratory**. Its many nodes keep the true cohort,
rewarded trajectory, unrewarded trajectory, shared noise, slow drift,
reward-correlated confounding, missing blocks, estimand, fits, inference, and
leave-one-mouse-out check separately inspectable.

Try these deliberately:

- set the two rates equal while keeping different plateaus;
- add shared drift to see why a within-session rise is not automatically learning;
- add a rewarded-only nuisance trend to manufacture a false group difference;
- increase missing blocks or mouse heterogeneity to see uncertainty widen;
- change block width, session length, early horizon, and cohort size together.

The simulation starts from synthetic mouse-level $d'$ trajectories rather than
raw calcium frames, so it tests the **analysis and inference layer**, not the
full neural estimator. Every plot says simulation; none of its values is data
evidence.
"""
            ),
            py(
                """
#@title Graph 3 transformations { display-mode: "form" }
@graph.node(outputs="simulation_cohort")
def define_simulation_cohort(
    rewarded_mouse_count=4,
    unrewarded_mouse_count=9,
    simulated_block_width=40,
    simulated_block_count=8,
):
    values = {
        "rewarded_mice": int(rewarded_mouse_count),
        "unrewarded_mice": int(unrewarded_mouse_count),
        "block_trials": int(simulated_block_width),
        "n_blocks": int(simulated_block_count),
    }
    if min(values.values()) < 2:
        raise ValueError("mouse and block counts must be at least two")
    return values


@graph.node(outputs="rewarded_truth")
def define_rewarded_truth(rewarded_initial_rate=0.018, rewarded_plateau=1.25):
    if float(rewarded_plateau) <= 0:
        raise ValueError("rewarded_plateau must be positive")
    return {"rate": float(rewarded_initial_rate), "plateau": float(rewarded_plateau)}


@graph.node(outputs="unrewarded_truth")
def define_unrewarded_truth(unrewarded_initial_rate=0.008, unrewarded_plateau=1.05):
    if float(unrewarded_plateau) <= 0:
        raise ValueError("unrewarded_plateau must be positive")
    return {"rate": float(unrewarded_initial_rate), "plateau": float(unrewarded_plateau)}


@graph.node(outputs="shared_truth")
def define_shared_simulation_truth(
    simulation_baseline=0.15,
    between_mouse_heterogeneity=1.0,
    simulation_seed=7,
):
    if float(between_mouse_heterogeneity) < 0:
        raise ValueError("between_mouse_heterogeneity must be non-negative")
    return {
        "baseline": float(simulation_baseline),
        "heterogeneity": float(between_mouse_heterogeneity),
        "seed": int(simulation_seed),
    }


@graph.node(outputs="latent_curves")
def simulate_latent_mouse_curves(simulation_cohort, rewarded_truth, unrewarded_truth, shared_truth):
    if min(rewarded_truth["plateau"], unrewarded_truth["plateau"]) <= shared_truth["baseline"]:
        raise ValueError("each simulated plateau must be above the shared baseline")
    rows = simulate_mouse_curves(
        rewarded_mice=simulation_cohort["rewarded_mice"],
        unrewarded_mice=simulation_cohort["unrewarded_mice"],
        rewarded_rate=rewarded_truth["rate"],
        unrewarded_rate=unrewarded_truth["rate"],
        rewarded_plateau=rewarded_truth["plateau"],
        unrewarded_plateau=unrewarded_truth["plateau"],
        baseline=shared_truth["baseline"],
        noise=0.0,
        block_trials=simulation_cohort["block_trials"],
        n_blocks=simulation_cohort["n_blocks"],
        seed=shared_truth["seed"],
    )
    output = []
    for group in (REWARDED, UNREWARDED):
        selected = [row for row in rows if row["group"] == group]
        group_mean = np.mean(np.stack([row["dprime"] for row in selected]), axis=0)
        for row in selected:
            truth = group_mean + shared_truth["heterogeneity"] * (row["dprime"] - group_mean)
            output.append({**row, "true_dprime": truth, "dprime": truth.copy()})
    return output


@graph.node(outputs="confounded_curves")
def inject_nonsensory_trends(
    latent_curves,
    shared_drift_per_100_trials=0.00,
    rewarded_nuisance_per_100_trials=0.00,
):
    rows = []
    for row in latent_curves:
        trial_scale = np.asarray(row["midpoint"], dtype=float) / 100.0
        nuisance = float(shared_drift_per_100_trials) * trial_scale
        if row["group"] == REWARDED:
            nuisance = nuisance + float(rewarded_nuisance_per_100_trials) * trial_scale
        rows.append({**row, "nuisance": nuisance, "dprime": row["true_dprime"] + nuisance})
    return rows


@graph.node(outputs="observed_curves")
def observe_noisy_incomplete_curves(
    confounded_curves,
    shared_truth,
    observation_noise=0.10,
    missing_block_fraction=0.00,
):
    noise = float(observation_noise)
    missing = float(missing_block_fraction)
    if noise < 0 or not 0 <= missing <= 0.45:
        raise ValueError("noise must be non-negative and missing fraction must be 0 to 0.45")
    rng = np.random.default_rng(shared_truth["seed"] + 1009)
    rows = []
    for row in confounded_curves:
        observed = np.asarray(row["dprime"], dtype=float) + rng.normal(0, noise, len(row["dprime"]))
        missing_count = min(int(round(missing * len(observed))), max(len(observed) - 4, 0))
        mask = np.zeros(len(observed), dtype=bool)
        if missing_count:
            mask[rng.choice(len(observed), size=missing_count, replace=False)] = True
            observed[mask] = np.nan
        rows.append({**row, "dprime": observed, "missing": mask})
    return rows


@graph.node(outputs="synthetic_estimand")
def define_synthetic_estimand(
    simulation_cohort,
    early_trial_horizon=140,
    minimum_early_blocks=3,
    simulation_bootstrap_draws=1200,
    simulation_alternative="greater",
):
    if simulation_alternative not in {"greater", "two-sided"}:
        raise ValueError("simulation_alternative must be greater or two-sided")
    return {
        "early_horizon": int(early_trial_horizon),
        "minimum_early_blocks": int(minimum_early_blocks),
        "bootstrap_draws": int(simulation_bootstrap_draws),
        "alternative": simulation_alternative,
        "block_trials": simulation_cohort["block_trials"],
    }


@graph.node(outputs="synthetic_rate_summaries")
def fit_synthetic_early_rates(observed_curves, synthetic_estimand):
    rows = []
    for row in observed_curves:
        slope = fit_early_slope(
            row["midpoint"], row["dprime"],
            early_horizon=synthetic_estimand["early_horizon"],
        )
        if slope["n_blocks"] < synthetic_estimand["minimum_early_blocks"]:
            slope = {**slope, "slope": float("nan"), "intercept": float("nan")}
        rows.append({**row, "slope": slope})
    return rows


@graph.node(outputs="synthetic_saturation_summaries")
def fit_synthetic_saturation(synthetic_rate_summaries):
    return [
        {**row, "saturation": fit_saturation_curve(row["midpoint"], row["dprime"])}
        for row in synthetic_rate_summaries
    ]


@graph.node(outputs="synthetic_inference")
def infer_synthetic_group_contrast(synthetic_saturation_summaries, synthetic_estimand):
    slopes = np.asarray([row["slope"]["slope"] for row in synthetic_saturation_summaries])
    groups = np.asarray([row["group"] for row in synthetic_saturation_summaries])
    mice = np.asarray([row["mouse"] for row in synthetic_saturation_summaries])
    valid_counts = {
        group: int(np.count_nonzero((groups == group) & np.isfinite(slopes)))
        for group in (REWARDED, UNREWARDED)
    }
    result = {
        "available": min(valid_counts.values()) >= 2,
        "slopes": slopes,
        "groups": groups,
        "mice": mice,
    ...
    if result["available"]:
        result["permutation"] = exact_group_permutation(
            slopes, groups, mouse_ids=mice,
            alternative=synthetic_estimand["alternative"],
        )
        result["bootstrap"] = bootstrap_group_difference(
            slopes, groups, mouse_ids=mice,
            n_boot=synthetic_estimand["bootstrap_draws"], seed=11,
        )
    return result


@graph.node(outputs="synthetic_leave_one_out")
def leave_one_mouse_out_synthetic(synthetic_inference):
    if not synthetic_inference["available"]:
        return []
    slopes = synthetic_inference["slopes"]
    groups = synthetic_inference["groups"]
    mice = synthetic_inference["mice"]
    rows = []
    for index, mouse in enumerate(mice):
        ...
        rows.append({
            "omitted_mouse": str(mouse),
            "omitted_group": str(groups[index]),
...
def plot_analysis_sandbox(synthetic_mouse_summaries, synthetic_inference):
    fig, axes = plt.subplots(2, 3, figsize=(13.4, 7.5), constrained_layout=True)

    for row in synthetic_mouse_summaries:
        axes[0, 0].plot(
            row["midpoint"], row["dprime"],
            color=GROUP_COLOURS[row["group"]], alpha=0.34, linewidth=1,
        )
    axes[0, 0].axvspan(0, 140, color="#f28e2b", alpha=0.10, label="example early horizon")
    axes[0, 0].set(title="Every synthetic mouse", xlabel="trial number", ylabel="held-out d′")
    axes[0, 0].legend(fontsize=7)

    for group in (REWARDED, UNREWARDED):
        selected = [row for row in synthetic_mouse_summaries if row["group"] == group]
        x = selected[0]["midpoint"]
        values = np.stack([row["dprime"] for row in selected])
        mean = np.mean(values, axis=0)
        sem = np.std(values, axis=0, ddof=1) / np.sqrt(len(values))
        axes[0, 1].plot(x, mean, marker="o", color=GROUP_COLOURS[group], label=group)
        axes[0, 1].fill_between(x, mean-sem, mean+sem, color=GROUP_COLOURS[group], alpha=0.16)
    ...
    for offset, group in enumerate((REWARDED, UNREWARDED)):
        selected = synthetic_inference["slopes"][np.asarray(synthetic_inference["groups"]) == group]
        x = offset + rng.normal(0, 0.035, size=len(selected))
        axes[0, 2].scatter(x, selected, color=GROUP_COLOURS[group], s=35, alpha=0.8)
    ...
    axes[0, 2].set(title="Primary estimand: one slope per mouse", ylabel="early Δd′ per trial")

    boot = synthetic_inference["bootstrap"]
    axes[1, 0].hist(boot["samples"], bins=35, color="#6b7280", alpha=0.75)
    axes[1, 0].axvline(0, color="#e15759", linestyle="--", label="no rate difference")
    axes[1, 0].axvline(boot["difference"], color="#111", linewidth=2, label="observed synthetic Δ")
    axes[1, 0].set(title="Mouse bootstrap", xlabel="rewarded - unrewarded slope", ylabel="bootstrap samples")
    axes[1, 0].legend(fontsize=7)

    for offset, group in enumerate((REWARDED, UNREWARDED)):
        selected = [row for row in synthetic_mouse_summaries if row["group"] == group]
        rates = [row["saturation"]["initial_rate"] for row in selected]
        plateaus = [row["saturation"]["plateau"] for row in selected]
        axes[1, 1].scatter(rates, plateaus, color=GROUP_COLOURS[group], label=group, alpha=0.8)
    axes[1, 1].set(title="Nonlinear secondary parameters", xlabel="fitted initial rate A·k", ylabel="fitted plateau b+A")
    axes[1, 1].legend(fontsize=8)

    axes[1, 2].axis("off")
    permutation = synthetic_inference["permutation"]
    lo, hi = boot["ci"]
    decision = "supports the directional prediction" if lo > 0 else "does not resolve the directional prediction"
    axes[1, 2].text(
        0, 1,
        "SIMULATION ONLY - NOT A DATA RESULT\\n\\n"
        f"Synthetic Δ slope: {boot['difference']:.4f} d′/trial\\n"
        f"Mouse-bootstrap 95% interval: [{lo:.4f}, {hi:.4f}]\\n"
        f"Exact label permutations: {int(permutation['permutations'])}\\n"
        f"One-sided permutation p: {permutation['pvalue']:.4f}\\n\\n"
        f"In this synthetic run, the interval {decision}.\\n\\n"
        "For real data, show all mouse curves and treat the\\n"
        "permutation as dependent on cohort-label exchangeability.",
        va="top", fontsize=9.2,
    )
    fig.suptitle("Graph 3 · SIMULATION ONLY · distinguish early rate from plateau")
    plt.close(fig)
    return fig


@graph.node(outputs="plots")
def make_simulation_plots(
    simulation_cohort,
    synthetic_estimand,
    synthetic_saturation_summaries,
    synthetic_inference,
    synthetic_leave_one_out,
    simulation_plot_percentile=98,
):
    percentile = float(simulation_plot_percentile)
    if not 80 <= percentile <= 100:
        raise ValueError("simulation_plot_percentile must be between 80 and 100")
    rows = synthetic_saturation_summaries
    fig, axes = plt.subplots(2, 4, figsize=(17.0, 7.7), constrained_layout=True)

    for row in rows:
        axes[0, 0].plot(
            row["midpoint"], row["true_dprime"],
            color=GROUP_COLOURS[row["group"]], alpha=0.18, linestyle="--",
        )
        axes[0, 0].plot(
            row["midpoint"], row["dprime"],
            color=GROUP_COLOURS[row["group"]], alpha=0.48, marker="o", markersize=2.5,
        )
    axes[0, 0].axvspan(
        0, synthetic_estimand["early_horizon"],
        color="#f28e2b", alpha=0.10, label="chosen early window",
    )
    axes[0, 0].set(
        title="Every mouse: truth (dashed) and observed",
        xlabel="trial number", ylabel="synthetic d′",
    )
    axes[0, 0].legend(fontsize=7)
    ...
    for group in (REWARDED, UNREWARDED):
        selected = [row for row in rows if row["group"] == group]
        values = np.stack([row["dprime"] for row in selected])
        truth = np.stack([row["true_dprime"] for row in selected])
        count = np.sum(np.isfinite(values), axis=0)
        mean = np.divide(
            np.nansum(values, axis=0), count,
        ...
        for index in range(values.shape[1]):
            ...
            if len(finite) > 1:
                sem[index] = np.std(finite, ddof=1) / np.sqrt(len(finite))
        x = selected[0]["midpoint"]
        axes[0, 1].plot(
            x, np.mean(truth, axis=0), color=GROUP_COLOURS[group],
            linestyle="--", alpha=0.7,
        )
        axes[0, 1].plot(
            x, mean, marker="o", color=GROUP_COLOURS[group],
            label=f"{group} observed",
        )
        axes[0, 1].fill_between(
            x, mean-sem, mean+sem, color=GROUP_COLOURS[group], alpha=0.14,
        )
    axes[0, 1].set(title="Group mean ± mouse SEM", xlabel="trial number", ylabel="synthetic d′")
    axes[0, 1].legend(fontsize=7)

    rng = np.random.default_rng(19)
    for offset, group in enumerate((REWARDED, UNREWARDED)):
        selected = synthetic_inference["slopes"][
            (synthetic_inference["groups"] == group)
            & np.isfinite(synthetic_inference["slopes"])
        ]
        axes[0, 2].scatter(
            offset + rng.normal(0, 0.035, len(selected)), selected,
            color=GROUP_COLOURS[group], s=34,
        )
        if len(selected):
            axes[0, 2].plot(
                [offset-0.16, offset+0.16], [np.mean(selected)]*2,
                color="black", linewidth=2,
            )
    axes[0, 2].axhline(0, color="#999", linewidth=1)
    axes[0, 2].set_xticks([0, 1], ["Rewarded", "Unrewarded"])
    axes[0, 2].set(title="One early slope per mouse", ylabel="Δd′ per trial")

    residual_matrix = np.stack([
        row["dprime"] - row["true_dprime"] for row in rows
    ])
    finite_abs = np.abs(residual_matrix[np.isfinite(residual_matrix)])
    limit = max(
        float(np.percentile(finite_abs, percentile)) if len(finite_abs) else 0.0,
        0.05,
    )
    image = axes[0, 3].imshow(
        residual_matrix, aspect="auto", cmap="coolwarm", vmin=-limit, vmax=limit,
    )
    axes[0, 3].set(
        title="Noise + nuisance + missingness",
        xlabel="time block", ylabel="synthetic mouse",
    )
    fig.colorbar(image, ax=axes[0, 3], label="observed - sensory truth")

    if synthetic_inference["available"]:
        boot = synthetic_inference["bootstrap"]
        axes[1, 0].hist(boot["samples"], bins=35, color="#6b7280", alpha=0.75)
        axes[1, 0].axvline(0, color="#e15759", linestyle="--")
        axes[1, 0].axvline(boot["difference"], color="#111", linewidth=2)
    else:
        axes[1, 0].text(
            0.5, 0.5, "Not enough mice with the\\nminimum early-block support",
            ha="center", va="center", transform=axes[1, 0].transAxes,
        )
    axes[1, 0].set(
        title="Mouse bootstrap", xlabel="rewarded - unrewarded slope", ylabel="draws",
    )

    for group in (REWARDED, UNREWARDED):
        selected = [row for row in rows if row["group"] == group]
        axes[1, 1].scatter(
            [row["saturation"]["initial_rate"] for row in selected],
            [row["saturation"]["plateau"] for row in selected],
            color=GROUP_COLOURS[group], label=group, alpha=0.8,
        )
    axes[1, 1].set(
        title="Rate and plateau are distinct",
        xlabel="fitted initial rate A·k", ylabel="fitted plateau b+A",
    )
    axes[1, 1].legend(fontsize=7)

    if synthetic_leave_one_out:
        differences = [row["difference"] for row in synthetic_leave_one_out]
        colours = [GROUP_COLOURS[row["omitted_group"]] for row in synthetic_leave_one_out]
        axes[1, 2].scatter(differences, np.arange(len(differences)), c=colours, s=24)
        axes[1, 2].axvline(0, color="#e15759", linestyle="--")
        axes[1, 2].set_yticks(
            np.arange(len(differences)),
            [row["omitted_mouse"] for row in synthetic_leave_one_out],
            fontsize=6,
        )
    else:
        axes[1, 2].text(
            0.5, 0.5, "Leave-one-out unavailable",
            ha="center", va="center", transform=axes[1, 2].transAxes,
        )
    axes[1, 2].set(title="Leave one mouse out", xlabel="remaining slope difference")

    axes[1, 3].axis("off")
    lines = [
        "SIMULATION ONLY - NOT A DATA RESULT",
        "",
        f"Cohort: {simulation_cohort['rewarded_mice']} rewarded + {simulation_cohort['unrewarded_mice']} unrewarded mice",
        f"Blocks: {simulation_cohort['n_blocks']} × {simulation_cohort['block_trials']} trials",
        f"Early rule: ≥{synthetic_estimand['minimum_early_blocks']} valid blocks through trial {synthetic_estimand['early_horizon']}",
        f"Valid slopes: {synthetic_inference['valid_counts']}",
    ]
    if synthetic_inference["available"]:
        boot = synthetic_inference["bootstrap"]
        perm = synthetic_inference["permutation"]
        lines += [
            f"Slope difference: {boot['difference']:.4f} d′/trial",
            f"Mouse-bootstrap 95% interval: [{boot['ci'][0]:.4f}, {boot['ci'][1]:.4f}]",
            f"Exact {synthetic_estimand['alternative']} p: {perm['pvalue']:.4f} ({int(perm['permutations'])} labels)",
        ]
    lines += [
        "",
        "A rewarded-only nuisance trend can mimic the prediction.",
        "Use this graph to learn failure modes, never to support the claim.",
    ]
    axes[1, 3].text(0, 1, "\\n".join(lines), va="top", fontsize=8.4)
    fig.suptitle(
        "Graph 3 · SIMULATION ONLY · rate, plateau, confounding, and uncertainty"
    )
    plt.close(fig)
    return fig
"""
            ...
            py(
                """
#@title Change the hypothetical dynamics, then run { display-mode: "form" }
sandbox_graph = graph.Graph(
    "Plan A design sandbox (simulation only)",
    define_simulation_cohort,
    define_rewarded_truth,
    define_unrewarded_truth,
    define_shared_simulation_truth,
    simulate_latent_mouse_curves,
    inject_nonsensory_trends,
    observe_noisy_incomplete_curves,
    define_synthetic_estimand,
    fit_synthetic_early_rates,
    fit_synthetic_saturation,
    infer_synthetic_group_contrast,
    leave_one_mouse_out_synthetic,
    make_simulation_plots,
)
sandbox_panel = sandbox_graph.widget(
    controls={
        "rewarded_mouse_count": widgets.IntSlider(description="Rewarded mice", value=4, min=3, max=6, step=1),
        "unrewarded_mouse_count": widgets.IntSlider(description="Unrewarded mice", value=9, min=6, max=12, step=1),
        "simulated_block_width": widgets.Dropdown(description="Trials per block", options=[32, 40, 50], value=40),
        "simulated_block_count": widgets.IntSlider(description="Number of blocks", value=8, min=6, max=12, step=1),
        "rewarded_initial_rate": widgets.FloatSlider(description="Rewarded rate", value=0.018, min=0.002, max=0.030, step=0.002, readout_format=".3f"),
        "unrewarded_initial_rate": widgets.FloatSlider(description="Unrewarded rate", value=0.008, min=0.002, max=0.030, step=0.002, readout_format=".3f"),
        "rewarded_plateau": widgets.FloatSlider(description="Rewarded plateau", value=1.25, min=0.6, max=2.0, step=0.05),
        "unrewarded_plateau": widgets.FloatSlider(description="Unrewarded plateau", value=1.05, min=0.6, max=2.0, step=0.05),
        "simulation_baseline": widgets.FloatSlider(description="Shared baseline", value=0.15, min=-0.2, max=0.5, step=0.05),
        "between_mouse_heterogeneity": widgets.FloatSlider(description="Mouse heterogeneity", value=1.0, min=0.0, max=2.0, step=0.25),
        "observation_noise": widgets.FloatSlider(description="Noise", value=0.10, min=0.02, max=0.30, step=0.02),
        "shared_drift_per_100_trials": widgets.FloatSlider(description="Shared drift/100", value=0.00, min=-0.15, max=0.20, step=0.025),
        "rewarded_nuisance_per_100_trials": widgets.FloatSlider(description="Rewarded nuisance/100", value=0.00, min=-0.15, max=0.20, step=0.025),
        "missing_block_fraction": widgets.FloatSlider(description="Missing blocks", value=0.00, min=0.0, max=0.40, step=0.10, readout_format=".1f"),
        "simulation_seed": widgets.IntSlider(description="Simulation seed", value=7, min=1, max=50, step=1),
        "early_trial_horizon": widgets.Dropdown(description="Early horizon", options=[100, 140, 180, 220], value=140),
        "minimum_early_blocks": widgets.Dropdown(description="Min early blocks", options=[2, 3, 4], value=3),
        "simulation_bootstrap_draws": widgets.Dropdown(description="Bootstrap draws", options=[400, 1200, 3000], value=1200),
        "simulation_alternative": widgets.Dropdown(description="Alternative", options=[("Rewarded > unrewarded", "greater"), ("Two-sided", "two-sided")], value="greater"),
        "simulation_plot_percentile": widgets.IntSlider(description="Residual colour %", value=98, min=90, max=100, step=1),
    },
    show="plots",
)
sandbox_panel
"""
    ...
    cells.extend(
        [
            md(
                r"""
## Graph 4 - Real Plan A: rewarded vs unrewarded within-session $d'$ dynamics

This is the graph that answers the project question. Start in **Plan only** to
see the exact sessions and budget. Then choose **Load and analyse** in Colab.

The default two-mice-per-group setting is a pipeline preview. It must not be
reported as the cohort result. For the primary analysis choose **All eligible
mice**: 4 rewarded and 9 unrewarded before-learning sessions.
...
2. Compare every selected field with the immutable primary signature. **Primary
   locked** mode refuses to run if even one scientific field differs.
...
7. Reduce to one rate per mouse. Preview inference is suppressed by default;
   the locked full cohort or an explicit exploratory override is required.
8. Bootstrap mice, enumerate exact labels, and leave one mouse out in separate
   inspectable nodes.

The primary path requires complete coverage of every selected corridor bin for
each trial. Also remember that the release-wide SVD/area basis is
...
def define_real_execution_policy(
    ...
    real_inference_alternative="greater",
    real_bootstrap_draws=4000,
    allow_exploratory_inference=False,
):
    if claim_status not in {"pipeline_preview", "primary_locked", "sensitivity"}:
...
def assemble_locked_analysis_spec(
    real_cohort_policy,
    representation_spec,
    real_contrast_spec,
    time_estimator_spec,
    ...
    deviations = {
        name: {"primary": primary[name], "selected": observed[name]}
        for name in primary if observed[name] != primary[name]
    }
    if execution_spec["claim_status"] == "primary_locked" and deviations:
        raise ValueError(
            "Primary locked run changed immutable fields: "
            + ", ".join(sorted(deviations))
        )
    complete = execution_spec["claim_status"] == "primary_locked" and not deviations
    return {
        **observed,
...
def choose_real_hypothesis_analysis(
    ...
    return {
        ...
        "load_mode": load_mode,
        "sessions": sessions,
        "complete_primary": training_stage == "before_learning" and int(cohort_size) == 0,
    }
...
def summarize_real_mouse_statistics_expanded(session_curves, analysis_spec):
    return {
        ...
        "complete_primary": analysis_spec["complete_primary"],
...
def infer_real_reward_rate_difference_expanded(mouse_statistics, analysis_spec):
    ...
    allowed = (
        analysis_spec["complete_primary"]
    ...
    return {
        ...
        "exploratory_override": not analysis_spec["complete_primary"],
    }
...
def plot_real_reward_hypothesis(
    ...
    if analysis_spec["load_mode"] == "plan_only":
        ...
        axes[1].text(
            ...
            "Use all eligible mice for the primary cohort result.",
            va="top", fontsize=10,
        )
    ...
    if group_inference.get("available"):
        ...
        lo, hi = boot["ci"]
        completeness = "COMPLETE PRIMARY COHORT" if analysis_spec["complete_primary"] else "PREVIEW / SENSITIVITY - NOT PRIMARY"
        ...
        axes[0, 3].text(
            ...
            f"One-sided exact-label p: {perm['pvalue']:.4f}\\n"
...
def make_real_plan_a_plots(
    ...
    if analysis_spec["execution"]["load_mode"] == "plan_only":
        ...
        axes[1, 1].axis("off")
        deviation_lines = ["IMMUTABLE PRIMARY SIGNATURE"]
        if analysis_spec["deviations"]:
            deviation_lines += [
                f"{name}: {value['selected']} (primary {value['primary']})"
                for name, value in analysis_spec["deviations"].items()
            ]
        else:
            deviation_lines += ["No scientific deviations."]
        deviation_lines += ["", "Primary locked mode refuses any changed field."]
...
hypothesis_panel = hypothesis_graph.widget(
    controls={
        "real_training_set": widgets.Dropdown(
            description="Training set",
            options=[("Train 1 (primary)", "train1"), ("Train 2 (sensitivity)", "train2")],
            value="train1",
        ),
        "training_stage": widgets.Dropdown(
            description="Training stage",
            options=[("Before learning (primary)", "before_learning"), ("After learning (mixed reward sensitivity)", "after_learning")],
            value="before_learning",
        ),
        "cohort_size": widgets.Dropdown(
            description="Cohort size",
            options=[("Two per group preview", 2), ("All eligible mice (primary)", 0), ("One per group smoke test", 1), ("Three per group preview", 3)],
            value=2,
        ),
        ...
        "cortical_area": widgets.Dropdown(
            description="Cortical area",
            options=[("V1 (proposal primary)", "V1"), ("Medial HVA (secondary)", "mHV"), ("Lateral HVA (secondary)", "lHV"), ("Anterior HVA (secondary)", "aHV")],
            value="V1",
        ),
        ...
        "movement_rule": widgets.Dropdown(
            ...
            options=[("Moving frames only (primary)", "moving_only"), ("All valid frames (sensitivity)", "all_valid_frames")],
            value="moving_only",
        ),
        "stimulus_pair": widgets.Dropdown(
            description="Stimulus pair",
            options=[("Leaf 1 vs circle 1 (primary)", "leaf1_vs_circle1"), ("Leaf 2 vs circle 2", "leaf2_vs_circle2")],
            value="leaf1_vs_circle1",
        ),
        "corridor_region": widgets.Dropdown(
            description="Corridor window",
            options=[("Texture 0-4 m (primary)", "texture_0_4"), ("Full corridor 0-6 m", "full_corridor_0_6"), ("Custom bounds", "custom")],
            value="texture_0_4",
        ),
        ...
        "real_coverage_rule": widgets.Dropdown(
            description="Coverage rule",
            options=[("Complete window (primary)", "complete"), ("Available bins (sensitivity)", "available")],
            value="complete",
        ),
        ...
        "block_stride_mode": widgets.Dropdown(
            ...
            options=[("Non-overlapping (primary)", "nonoverlap"), ("Half-block overlap (display)", "half_block_overlap")],
            value="nonoverlap",
        ),
        ...
        "claim_status": widgets.Dropdown(
            ...
            options=[("Pipeline preview (default)", "pipeline_preview"), ("Primary locked (exact signature)", "primary_locked"), ("Sensitivity / exploratory", "sensitivity")],
        ...
        "real_inference_alternative": widgets.Dropdown(
            ...
            options=[("Rewarded > unrewarded", "greater"), ("Two-sided", "two-sided")],
            value="greater",
        ),
...
After the complete V1 primary run, use `hypothesis_graph.run_many(...)` for a
small, declared grid rather than choosing the prettiest curve:
"""
            ),
            py(
                """
...
"""
    ...
    cells.extend(
        [
            md(
                """
## Graph 5 - Plan B: corridor position, behavior, and the location of change

Plan B is both a fallback question and a necessary control for Plan A. A change
in corridor-averaged $d'$ can be localized to visual texture, cue, reward,
licking, or grey space only after looking along position.
...
**Reuse last real Plan A run**. This passes the documented
...
def make_plan_b_plots(
    ...
    lines = [
        "PLAN B - POSITION IS A REPEATED MEASURE",
        "",
...
"""
            ...
            md(
                """
### Plan B interpretation guide

- A texture-confined effect (0-4 m) that survives speed/support controls is more
  consistent with sensory discriminability than an effect centered only on the
  reward/cue/lick zone.
- A grey-only or whole-corridor drift suggests global state, bleaching, motion,
  or a non-sensory coding change.
- A stable cross-temporal axis with increasing d′ supports sharpening along an
  existing representation; poor early-to-late generalization suggests axis
  rotation/reorganization.
- Cue, reward and lick variables are partly consequences of reward. Adjusting
  them in the primary total-effect estimate can remove the mechanism itself;
  use pre-cue windows and behavioral adjustment as named sensitivity analyses.
- Position profiles are repeated measurements within mouse. Formal uncertainty
  must resample mice and control the whole position curve, not attach a separate
  uncorrected p-value to each bin.
"""
            ),
        ]
    )
...
Before interpreting a positive result, check all of these:

- the V1 primary outcome was declared before opening the group result;
...
- mHV/lHV/aHV results are labelled secondary and multiplicity is addressed;
- a fitted plateau is reported as “not observed” when less than 90% of the
  fitted amplitude is attained or the rate fit hits its search-grid boundary;
...
The clean conclusion format is: effect estimate first, mouse-level uncertainty
second, model/eligibility choices third, and limitations in the same paragraph.
"""
            ),
        ]
    )
