...
def _panel_parts(panel):
    ...
    button = next(
        item for item in toolbar.children if isinstance(item, widgets.Button)
    )
    status = next(item for item in toolbar.children if isinstance(item, widgets.HTML))
...
def test_named_ports_fan_out_and_run_in_declared_order():
    ...
    assert run.get("summary") == 21
    assert run.get("missing", "fallback") == "fallback"
    assert dict(run) == dict(run.outputs)
    assert run.final == 21
...
def test_graph_objects_export_bounded_programmer_facing_dictionaries():
    class ArrayLike:
        shape = (452, 18, 48)
        dtype = "float32"

    @graph.node(outputs="features")
    def load(scale=2):
        return ArrayLike()

    flow = graph.Graph("inspectable", load)
    run = flow.run()

    assert load.to_dict()["outputs"] == ("features",)
    assert flow.to_dict() == flow.describe()
    assert run.to_dict()["outputs"] == {
        "features": "ArrayLike · 452×18×48 · float32"
    }
    assert run.to_dict()["settings"] == {"scale": 2}
    assert run.to_dict()["actual_values"] == "dict(run)"
    assert isinstance(dict(run)["features"], ArrayLike)
    assert "452×18×48" in repr(run)
    assert "features" in flow._repr_html_()
    assert "input_ports" in repr(flow)
...
def test_graph_explains_nodes_left_over_from_a_module_refresh():
    stale_node_type = type("Node", (), {"__module__": "graph"})

    with pytest.raises(graph.GraphError, match="earlier graph module instance"):
        graph.Graph("stale", stale_node_type())
...
def test_output_contract_and_node_failure_keep_context():
    ...
    assert error.value.to_dict()["node_name"] == "fails"
    assert "'node_name': 'fails'" in repr(error.value)
...
def test_widget_combines_a_wired_map_with_native_controls_and_output():
    ...
    assert isinstance(output, widgets.HTML)
    assert panel.result_view is output
    button.click()
    ...
    assert "graph-result-view" in output.value
    assert ">6</pre>" in output.value
...
def test_widget_run_to_lists_every_node_and_executes_only_that_dependency_path():
    widgets = pytest.importorskip("ipywidgets")
    calls = []
    ...
    @graph.node(outputs="summary")
    def summarize(selected, suffix="done"):
        calls.append("summarize")
        return f"{selected} {suffix}"

    flow = graph.Graph("partial flow", load, select, summarize)
    panel = flow.widget()
    toolbar = panel.children[0]
    run_to = next(
        item
        for item in toolbar.children
        if isinstance(item, widgets.Dropdown) and item.description == "Run to"
    )
    inspect_port = next(
        item
        for item in toolbar.children
        if isinstance(item, widgets.Dropdown)
        and item.description == "Inspect port"
    )
    _, _, _, button, status, _ = _panel_parts(panel)

    assert list(run_to.options) == [
        ("All", None),
        ("1 · Load", "load"),
        ("2 · Select", "select"),
    ...
    assert panel.last_run.order == ("load", "select")
    assert set(panel.last_run.outputs) == {"data", "selected"}
    assert dict(panel.last_run.settings) == {"scale": 2, "offset": 1}
    assert panel.last_error is None
    assert panel.run_count == 1
    assert inspect_port.disabled is False
    assert [value for _, value in inspect_port.options] == [
        None,
        "data",
        "selected",
    ]
    assert "summary" not in [value for _, value in inspect_port.options]
    assert "Stopped after Select" in status.value


def test_widget_inspects_any_produced_port_without_rerunning():
    widgets = pytest.importorskip("ipywidgets")
    calls = []
    panel = _example_graph(calls).widget(show="data")
    toolbar = panel.children[0]
    inspect_port = next(
        item
        for item in toolbar.children
    ...
    _, _, _, button, status, output = _panel_parts(panel)

    button.click()

    run = panel.last_run
    assert ">2</pre>" in output.value
    assert calls == ["load", "select", "summarize"]
    assert [value for _, value in inspect_port.options] == [
        None,
        "data",
    ...
    inspect_port.value = "selected"
    assert ">3</pre>" in output.value
    inspect_port.value = "summary"
    assert ">6</pre>" in output.value
    inspect_port.value = None

    assert ">2</pre>" in output.value
    assert calls == ["load", "select", "summarize"]
    assert panel.last_run is run
    assert panel.last_error is None
    assert panel.run_count == 1
    assert "no nodes reran" in status.value


def test_widget_clears_stale_output_before_running_and_after_input_changes():
    result_values_seen_during_run = []
    result_view = None

    @graph.node(outputs="value")
    def calculate(scale=1):
        result_values_seen_during_run.append(result_view.value)
        return scale

    panel = graph.Graph("clear output", calculate).widget(
        controls={"scale": [1, 2]}
    )
    surface, _, _, button, _, result_view = _panel_parts(panel)
    scale = next(
        item
        for item in _walk_widgets(surface)
    ...
    button.click()
    assert result_view.value
    button.click()
    scale.value = 2

    assert result_values_seen_during_run == ["", ""]
    assert result_view.value == ""
    assert panel.last_run is None
...
def test_widget_runs_and_shows_the_result_when_the_diagram_cannot_render(monkeypatch):
    flow = _example_graph()
    panel = flow.widget(controls={"scale": [1, 2]})
    surface, _, controls, button, status, output = _panel_parts(panel)
    ...
    assert ">6</pre>" in output.value
    assert "completed and the result is shown" in status.value
    assert "diagram unavailable" in status.value
    assert button.disabled is False
    assert all(control.disabled is False for control in controls)


def test_widget_keeps_a_valid_run_when_only_result_display_fails():
    widgets = pytest.importorskip("ipywidgets")
    ...
    @graph.node(outputs=("broken", "good"))
    def produce():
        return {"broken": BrokenHTML(), "good": 3}

    panel = graph.Graph("display failure", produce).widget(show="broken")
    toolbar = panel.children[0]
    inspect_port = next(
        item for item in toolbar.children
        if isinstance(item, widgets.Dropdown)
        and item.description == "Inspect port"
    )
    _, _, _, button, status, output = _panel_parts(panel)
    ...
    assert isinstance(panel.last_run["broken"], BrokenHTML)
    assert panel.run_count == 1
    assert isinstance(panel.last_error, RuntimeError)
    ...
    inspect_port.value = "good"

    assert ">3</pre>" in output.value
    assert panel.last_error is None
    assert "port good from run 1" in status.value


def test_widget_renders_html_svg_png_and_bounded_safe_plain_text():
    display_module = pytest.importorskip("IPython.display")
    widgets = pytest.importorskip("ipywidgets")

    class SVGResult:
        def _repr_svg_(self):
            return "<svg data-result='svg'><circle r='2'/></svg>"

    class LongResult:
        def __repr__(self):
            return "<unsafe>" + "x" * 20_000

    class BrokenTextResult:
        def __repr__(self):
    ...
    def produce():
        return {
            "html_result": display_module.HTML("<strong data-result='html'>ready</strong>"),
            "svg_result": SVGResult(),
            "png_result": display_module.Image(
                data=b"\x89PNG\r\n\x1a\nimage-data", format="png"
            ),
            "text_result": LongResult(),
            "broken_text_result": BrokenTextResult(),
        }

    panel = graph.Graph("rich results", produce).widget(
        show="html_result", auto_run=True
    )
    toolbar = panel.children[0]
    inspector = next(
        item
    ...
    output = panel.result_view

    assert "<strong data-result='html'>ready</strong>" in output.value

    inspector.value = "svg_result"
    assert "<svg data-result='svg'>" in output.value

    inspector.value = "png_result"
    assert "data:image/png;base64," in output.value
    encoded = output.value.split("data:image/png;base64,", 1)[1].split("'", 1)[0]
    assert base64.b64decode(encoded).startswith(b"\x89PNG\r\n\x1a\n")

    inspector.value = "text_result"
    assert "<unsafe>" not in output.value
    assert "&lt;unsafe&gt;" in output.value
    assert "…</pre>" in output.value
    assert len(output.value) < 13_000

    inspector.value = "broken_text_result"
    assert "BrokenTextResult (representation unavailable)" in output.value
    assert panel.last_error is None


def test_widget_replaces_oversized_rich_payloads_with_a_visible_message(monkeypatch):
    monkeypatch.setattr(graph, "_RESULT_MARKUP_LIMIT", 32)
    monkeypatch.setattr(graph, "_RESULT_PNG_LIMIT", 8)

    class HugeHTML:
        def _repr_html_(self):
            return "<strong>" + "x" * 40 + "</strong>"

    class HugePNG:
        def _repr_png_(self):
            return b"012345678"

    @graph.node(outputs=("html_result", "png_result"))
    def produce():
        return {"html_result": HugeHTML(), "png_result": HugePNG()}

    panel = graph.Graph("bounded results", produce).widget(
        show="html_result", auto_run=True
    )
    widgets = pytest.importorskip("ipywidgets")
    inspector = next(
    ...
    assert "HTML result omitted" in panel.result_view.value
    inspector.value = "png_result"
    assert "PNG result omitted" in panel.result_view.value
    assert panel.last_error is None


def test_widget_auto_run_is_explicit_and_uses_visible_defaults():
    calls = []

    @graph.node(outputs="value")
    def calculate(scale=2):
        calls.append(scale)
        return scale * 3

    flow = graph.Graph("automatic run", calculate)
    idle = flow.widget(auto_run=False)

    assert idle.last_run is None
    assert calls == []

    panel = flow.widget(controls={"scale": [4, 7]}, auto_run=True)

    assert calls == [4]
    assert panel.last_run["value"] == 12
    assert panel.run_count == 1
    assert ">12</pre>" in panel.result_view.value
...
def test_widget_closes_only_matplotlib_figures_created_by_each_run():
    ...
    _, _, _, button, _, output = _panel_parts(panel)
...
def test_diagram_shortens_opposing_port_labels_but_keeps_full_tooltips():
    @graph.node(outputs="source_payload_with_long_name")
    def source():
        return [1, 2, 3]

    @graph.node(outputs="result_payload_with_long_name")
    def transform(source_payload_with_long_name):
        return source_payload_with_long_name

    flow = graph.Graph("long labels", source, transform)
    markup = flow._diagram_html(flow.run(), canvas_only=True)
    full_input = "source_payload_with_long_name"
    visible_input = flow._fit_port_label(full_input, shared_row=True)
    full_output = "result_payload_with_long_name = list · 3 items · result"
    visible_output = flow._fit_port_label(full_output, shared_row=True)

    assert visible_input.endswith("…")
    assert visible_input in markup
    assert f"<title>{full_input}</title>" in markup
    assert visible_output.endswith("…")
    assert visible_output in markup
    assert f"<title>{full_output}</title>" in markup
