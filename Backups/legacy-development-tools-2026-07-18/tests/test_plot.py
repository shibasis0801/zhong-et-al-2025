...
def test_root_facade_and_recipe_catalog_expose_the_same_stable_api(monkeypatch):
    ...
    assert plot.guide().to_dict()["recipes"][0]["name"] == "curve"
    assert "recipes" in plot.guide()._repr_html_()
...
def test_recipes_return_fresh_tagged_figures_without_mutating_rcparams():
    ...
    try:
        ...
        assert plot.info(first).to_dict()["recipe"] == "curve"
        assert "'recipe': 'curve'" in repr(plot.info(first))
        assert "recipe" in plot.info(first)._repr_html_()
